...
TEST_CASE("Parsing numbers is correct", "[Parser]") {
  Number n(123);
  auto [value, str] = Number::Parse("123").value();
  REQUIRE(*(Number *)value == n);
  REQUIRE(str == "");
}

TEST_CASE("Parsing multiply", "[Parser]") {
  REQUIRE(Operator::Parse("*") == optional(make_pair(Operator(Operator::Multiply), (string) "")));
}
TEST_CASE("Parsing divide", "[Parser]") {
  REQUIRE(Operator::Parse("/") == optional(make_pair(Operator(Operator::Divide), (string) "")));
}
TEST_CASE("Parsing subtract", "[Parser]") {
  REQUIRE(Operator::Parse("-") == optional(make_pair(Operator(Operator::Subtract), (string) "")));
}
TEST_CASE("Parsing add", "[Parser]") {
  REQUIRE(Operator::Parse("+") == optional(make_pair(Operator(Operator::Add), (string) "")));
}
